...
//007301 申请平台专家
router.post('/applyPlatformTutors', (req, res) => {
    let path = config.allUrlSet.url.platformPort1 + "match-boot/admin/demand/applyPlatformTutors";
    apiHelper.getPlatformApi(path, null, req, res).then((data) => {
        res.send(data);
    });
});
...
//007209 分页获取申请专家记录
router.post('/pageApplyPlatformTutors', (req, res) => {
    let path = config.allUrlSet.url.platformPort1 + 'match-boot/admin/demand/pageApplyPlatformTutors';
    apiHelper.getPlatformApi(path, null, req, res).then((data) => {
        res.send(data);
    });
});
...
//007210 处理需求
router.post('/dealDemand', (req, res) => {
    let path = config.allUrlSet.url.platformPort1 + 'match-boot/admin/demand/dealDemand';
    apiHelper.getPlatformApi(path, null, req, res).then((data) => {
        res.send(data);
    });
});
