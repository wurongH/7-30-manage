    ...
    <Row>
      <Col span="24">
        <Card v-if="baseMessageShow">
            <div class="title">
                <span>参赛授权</span>
                <span>请在这里输入本届有资格向上级赛事提交作品的选手的手机号码，输入后该选手登录系统后可在<i style="color:red">个人中心</i>提交上级赛事作品</span>
            </div>
            <div class="tel">
            ...
            <Row justify="center">
              <Table border :loading="loading" :data="tableData" :columns="columns" @on-selection-change="selectionChange">
                <template slot-scope="{ row, index }" slot="action" v-if="row.submitStatus == 1">
                  <span class="text" @click="showModel('isShowRanking', row)">修改排名</span>
                  <span class="text" @click="getViewProjectInfo(row.entryId)" v-if="row.title != '暂无'">查看</span>
                  <span class="text" @click="showModel('isShowReview', row)">{{row.authorCheckStatus ? '取消' : ''}}审核</span>
                  <span class="text" @click="showModel('isShowReport', row)" v-if="row.authorCheckStatus">{{row.sendStatus ? '取消' : ''}}上报</span>
                </template>
              </Table>
            </Row>
            <Row>
    ...
    <Modal v-model="isShowRanking" title="修改排名" width="400" class="ranking-modal"
      ...
      <Form ref="rankingForm" :model="rankingForm" :rules="rankingRule" inline>
        <FormItem prop="sortNum">
            <Input type="text" @on-keyup="keyup()" v-model="rankingForm.sortNum" placeholder="请输入排名"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
...
<script>
  ...
  export default {
      ...
      methods:{
        keyup(){
          if (!this.rankingForm.sortNum.match(/^[\+\-]?\d*?\d*?$/))
            this.rankingForm.sortNum = this.t_value;
          else
            this.t_value = this.rankingForm.sortNum;
          if (this.rankingForm.sortNum.match(/^(?:[\+\-]?\d+?)?$/))
            this.o_value = this.rankingForm.sortNum;
        },
        handleCancel(){
