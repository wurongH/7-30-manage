...
<template>
  <div>
    ...
    <Modal v-model="messageShow" title="赛事信息" :width="700">
      <Form :label-width="100" ref="message" :model="message" :rules="ruleValidate">
        <FormItem label="届数名称" prop="sessionName" v-if="!edit">
            <Input v-model="message.sessionName" :maxlength="14" placeholder="输入届数名称" style="width:200px"  />
        </FormItem>
        <FormItem label="届数名称" v-if="edit">
            <Input v-model="editSessionName" :clearable="true" />
        </FormItem>
        <FormItem label="赛事名称" prop="matchName">
          <Input v-model="message.matchName" :maxlength="14" :clearable="true" placeholder="输入赛事名称"/>
        </FormItem>
        ...
        <FormItem label="评审须知" prop="notice">
          <textarea v-model="message.notice"  class="tinymce-textarea" id="tinymceEditerMessage" style="height:
...
export default {
  data() {
    return {
              ...
              h(
                "a",
                {
                  ...
                  on: {
                    click: () => {
                      this.message = {
                        sessionName: params.row.sessionName,
                      ...
                      this.edit = true;
                      this.editSessionName = this.message.sessionName;
                      console.log(this.message.sessionName);
                      this.spinShow = true;
      ...
      ruleValidate: {
        sessionName: [
            {required: true, message: '届数名称不能为空', trigger: 'blur'}
        ],
        matchName: [
  ...
  methods: {
        ...
        if (valid) {
          ...
          api(
            ...
            "/main/updateMatch",
            {
              ...
              sessionName: this.editSessionName,
            },
            res => {
    ...
    newMatch(){
        ...
        this.$refs['message'].validate((valid) => {
            if (valid) {
                ...
                if(text == "" || text == undefined || text == null || (text.length>0 &&text.trim().length == 0)) {
                    this.$Message.error('评审须知不能为空')
                    return false
                 }
                this.buttonLoading = true;
                api(this,'/main/newMatch',{
                    ...
                    levelId:this.$store.state.userInfo.levelId,
                    manageId:this.$store.state.userInfo.manageId,
                    sessionName: this.message.sessionName,
                    matchName:this.message.matchName,
                    endFlag:this.message.endFlag==true?0:1,
                    notice:this.message.notice,
                },res => {
    ...
    tinyInit(vm) {
      ...
      tinymce.init({
        ...
        toolbar1:
          "preview | undo redo | insert | styleselect | forecolor backcolor bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image emoticons media codesample",
        ...
        setup: function(editor) {
          editor.on("init", function(e) {
            vm.spinShow = false;
            if (localStorage.editorContentMessage && !vm.isAdd) {
              tinymce
                .get("tinymceEditerMessage")
                .setContent(localStorage.editorContentMessage);
            } else {
          ...
          editor.on("keydown", function(e) {
            localStorage.editorContentMessage = tinymce
              .get("tinymceEditerMessage")
              .getContent();
          });
