...
<template>
  <div>
        ...
        <Card v-if="baseMessageShow">
          ...
          <Row justify="center">
              <Table border :loading="loading" :data="tableData" :columns="columns" @on-selection-change="selectionChange">
                ...
                <template v-for="i in 10" slot-scope="{ row, index }" :slot="`tutor${i}`">
                  <template>
                    <Tag v-if="row[`tutor${i}`] && row[`tutor${i}`] != ''" @on-close="showDel(row[`tutorAuditId${i}`])" color="primary" closable>{{row[`tutor${i}`]}}</Tag>
                  </template>
                </template>
              </Table>
          </Row>
    ...
    <!-- 指派弹窗 -->
    <task ref="task" :value.sync="isTask" :selectTbaleData.sync="selectData" :assignFirstFlag="assignFirstFlag" :matchId="matchId" @updata-data="getList"></task>
  </div>
</template>
